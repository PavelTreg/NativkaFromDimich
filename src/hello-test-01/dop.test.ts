...
import {addMoneyToBudget, messageForPeople, repairHouse, toFireStaff, toHireStaff} from "./dop";
...
test("I want message", () => {
    toHireStaff(city.governmentBuildings[0], 20);
 const result = messageForPeople(city)
    expect(result).toBe(' Hello New York, with 1000000 people')
});
